...
using Keboo.Web.Proxy.Compression;
...
using Keboo.Web.Proxy.StreamExtended.BufferPool;
using Keboo.Web.Proxy.StreamExtended.Network;
...
internal class HttpStream : Stream, IHttpStreamWriter, IHttpStreamReader, IPeekStream
{
 ...
 default)
    ...
    internal async ValueTask WriteAsync(byte[] data, bool flush = false, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            await BaseStream.WriteAsync(data, cancellationToken);
    ...
    private static Task HandleBodyWrite(IHttpStreamWriter writer, bool isChunked, long contentLength,
